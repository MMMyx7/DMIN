...
def parse_arguments():
    ...
    parser.add_argument('--audio_semantic_feats_path', type=str, default='audio_desc_feats.pkl', help="The directory of the audio semantic features.")

    parser.add_argument('--video_semantic_feats_path', type=str, default='video_desc_feats.pkl', help="The directory of the video semantic features.")
...
def run(args, debug_args=None):
    ...
    if args.train:
        ...
        if getattr(args, "save_model", False):
            import torch
            ckpt_dir = os.path.join(args.output_path, "ckpts")
            os.makedirs(ckpt_dir, exist_ok=True)

            # Try to automatically get the model object
            model_obj = getattr(method, "model", None) or getattr(method, "net", None) or getattr(method, "backbone",
                                                                                                  None)
            if model_obj is None:
                raise RuntimeError("No model object found (tried .model / .net / .backbone)")

            # Use seed in filename to avoid overwrite
            save_path = os.path.join(ckpt_dir, f"final_{args.method}_seed{args.seed}.pth")
            torch.save(model_obj.state_dict(), save_path)
            logger.info(f"[SAVE] Final model weights saved to: {save_path}")
